    ...
    async def relay_message():
        async for message in websocket:
            if public_ws and public_ws.open:
                ...
                print(Fore.RED + "[ERROR] 公開サーバーに接続できません\n        サーバーの再起動をお試しください")
...
async def create_websocket():
    ...
    while True:
        try:
            ...
            status(2, False)
            print(Fore.RED + f"公開サーバーへの接続に失敗しました: {e}\n再接続を試みます...\n" + Fore.YELLOW + "Ctrl + Cでサーバーを終了します")
    ...
    async def monitor_connection():
        ...
        while True:
            if public_ws.closed:
                status(2, False)
                print(Fore.RED + "公開サーバーとの接続が切れました\n自動で再接続を試みます...\n" + Fore.YELLOW + "Ctrl + Cでサーバーを終了します")
                await asyncio.sleep(2)
