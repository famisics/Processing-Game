...
void VP_update() {
  ...
  text(doubleToJp(DATA_ENERGY), float(SB_blockWindowWidth) * 103 / 100, float(GAME_height) * 24.5 / 100);
  text(doubleToJp(SB_inflationRate * SB_inflationRateTemporary) + "倍", float(SB_blockWindowWidth) * 127 / 100, float(GAME_height) * 24.5 / 100);
  text(NET_channel, float(SB_blockWindowWidth) * 127 / 100, float(GAME_height) * 34.5 / 100);
  textSize(GAME_width / 58);
  text("Lv. " + doubleToJp(Math.ceil(SB_inflationRate / 1000)), float(SB_blockWindowWidth) * 103 / 100, float(GAME_height) * 34.5 / 100);
  textFont(fontMdsm);
